...
function registerMetaRoutes(app, {
  ...
  const adsLookupSeen = new Map();
  const ADS_LOOKUP_TTL_MS = 10 * 60 * 1000;
  ...
  async function runAdsLookup({ wa_id, wamid, inboundPhoneNumberId, m }) {
    try {
      ...
      console.log('[META][ADS][LOOKUP][START]', { wa_id, wamid, ad_id: adId, inboundPhoneNumberId: inboundPhoneNumberId || null });
      ...
      if (!out) {
        console.log('[META][ADS][LOOKUP][NO_DATA]', { wa_id, wamid, ad_id: adId });
        return;
      }
      ...
      console.log('[META][ADS][LOOKUP][OK]', { wa_id, wamid, ad_id: adId });
    } catch (e) {
      console.log('[META][ADS][LOOKUP][ERR]', { wa_id, wamid, message: e?.message || 'err' });
    }
  }
          ...
          for (const m of msgs) {
                ...
                if (st) {
                  if (!st.meta_ads_inflight) {
                    st.meta_ads_inflight = Promise.resolve()
                      .then(() => runAdsLookup({ wa_id, wamid, inboundPhoneNumberId, m }))
                      .catch(() => null)
                      .finally(() => {
                  ...
                  runAdsLookup({ wa_id, wamid, inboundPhoneNumberId, m }).catch?.(() => { });
                }
              } catch {
                (async () => { await runAdsLookup({ wa_id, wamid, inboundPhoneNumberId, m }); })();
              }
            }
            ...
            if (shouldEnqueueAsText) {
              try {
                ...
                if (clean) lead?.enqueueInboundText?.({ wa_id, inboundPhoneNumberId, text: clean, wamid });
              } catch { }
            }
