...
async function sendDocument(contato, url, captionOrOpts, opts = {}) {
  if (typeof captionOrOpts === 'object' && captionOrOpts) {
    opts = captionOrOpts;
    captionOrOpts = undefined;
  }
  ...
  const caption = String(captionOrOpts || '').trim();
  ...
  try {
    ...
    const documentObj = { link };
    if (caption) documentObj.caption = caption;
    if (opts.filename) documentObj.filename = String(opts.filename);
