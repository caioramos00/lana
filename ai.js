...
function createAiEngine({ db, sendMessage, aiLog = () => {} } = {}) {
  ...
  function logAiRequest({ wa_id, inboundPhoneNumberId, facts, historicoStr, msgParaPrompt, rendered, model }) {
    // limites ajustáveis por ENV (mas com defaults)
    const histMax = Number(process.env.AI_LOG_MAX_HIST || 2500);
    const msgMax = Number(process.env.AI_LOG_MAX_MSG || 2500);
    const factsMax = Number(process.env.AI_LOG_MAX_FACTS || 4000);
    ...
    // “messages” reais do Venice (system omitido)
