...
function createAiEngine({ db, sendMessage, aiLog = () => { } } = {}) {
    ...
    function shouldLogRequest() {
        // liga/desliga via ENV sem mexer no código
        // ex: AI_LOG_REQUEST=1
        return String(process.env.AI_LOG_REQUEST || '').trim() === '1';
    }

    function getLogLimits() {
        return {
            histMax: Number(process.env.AI_LOG_MAX_HIST || 2500),
            msgMax: Number(process.env.AI_LOG_MAX_MSG || 2500),
            factsMax: Number(process.env.AI_LOG_MAX_FACTS || 4000),
        };
    }
    ...
    async function handleInboundBlock({
        ...
        // ===== LOG: o que está indo pra IA (sem printar o system inteiro) =====
        if (shouldLogRequest()) {
            const { histMax, msgMax, factsMax } = getLogLimits();

            aiLog(`[AI][REQUEST][${wa_id}]`);
            ...
            // mostra o “envelope” que vai pro Venice (sem vazar o system prompt)
            aiLog(`[AI][REQUEST][${wa_id}] VENICE_MESSAGES:`);
            aiLog(JSON.stringify([
                {
                    role: 'system',
                    content: `[OMITTED] chars=${(rendered || '').length} sha256=${sha256Of(rendered || '')}`,
                },
                {
                    role: 'user',
                    content: 'Responda exatamente no formato JSON especificado.',
                },
            ], null, 2));
        }
